    ...
    m_model->createRow(dev);
     ...
     //   connect(but_save_set,SIGNAL(clicked()),this, SLOT(save_device_settings()));
        connect(but_create_set,SIGNAL(clicked()),this, SLOT(create_device_settings()));
         ...
         ui->tableView->setIndexWidget(m_model->index(m_model->row()-1 ,7),but_create_set);
        ui->tableView->setIndexWidget(m_model->index(m_model->row()-1,8),but_save_set);
...
//        m_client.setPassphrase(ui->tableView->item(i,3)->text());
...
{     
    m_model->delCurRow(ui->tableView->currentIndex().row());
   // ui->tableView
    if(m_model->row() == 0)
    {
        ui->but_del_device->setEnabled(false);
